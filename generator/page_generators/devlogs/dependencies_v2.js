...
exports.getPageGenerator = function(){
	return new (require("../../PageGenerator.js").PageGenerator)(
		...
		(page) => {
			page.paragraph(`In ${html.link("my first devlog", "/site/devlog/new_systems_requires_major_changes.html")}, I discussed how implementing Order Independent Declarations and the Panther build system required redoing a lot of work for the compiler as well as adding a new stage (Dependency Analysis). Update ${page.pcitCppVersion("v0.0.79.0")} contained the first proof of concept of this new system working. The code required to get it working was complex and difficult to understand, but the system worked and it seemed robust. There was a looming sense of difficulty coming down the line (such as pointers to user-defined types - more on that later), but I plowed through as this was the solution I came up with and I was sticking to it. Through ${page.pcitCppVersion("v0.0.80.0")} and ${page.pcitCppVersion("v0.0.81.0")} it continued to work well, albeit with more and more complexity. I then got to implementing ${html.highlight("@import")} and ran face-first into a wall made of pure abstract complexity.`);


			page.paragraph(`The original idea for the Dependency Analysis stage was that it would create a dependency graph of all of the symbols for the entire project. The graph couldn't be made in one go, however, as information only available through semantic analysis would be necessary to do so. For example, until the compiler knows which file an ${html.highlight("@import")} call is importing, there would be no way to generate the graph that stemmed from that. This solution was to do this later when it would have more information. Conceptually this seems simple, however the way the AST is purposely laid out makes it difficult to save/organize the information needed to facilitate this. Possible, yes - but difficult to implement cleanly, memory intensive, and slow. Part of the reason for building this new system was that it would increase compilation speed. There had to be a better way.`);
			 
			page.paragraph(`If I were to come up with a new solution, it would have to solve both this problem as well as other problems such as the pointers to user-defined types issue I mentioned earlier. In C/C++, if I have a variable of type ${html.highlight("Foo*")} when type ${html.highlight("Foo")} hasn't been defined yet, it's fine as long as the declaration of ${html.highlight("Foo")} exists. Ideally the Panther compiler would do something similar. However, for any given function, there would be no way to know if the definition of ${html.highlight("Foo")} was needed or just having the declaration is sufficient. If the pointer to ${html.highlight("Foo")} never got dereferenced, then semantic analysis wouldn't need to know the definition. In addition, what if a ${html.highlight("Foo*")} was returned from a function and was immediately dereferenced - how would dependency analysis know that this occurs without semantic analysis? So either the compiler wouldn't be able to use this neat trick, or it would have to go through a similar process like ${html.highlight("@import")} (deferring dependency graph generation), further increasing the complexity and memory usage, and decreasing the performance.`);
			...
			page.paragraph(`The new system now has the name "Symbol Process Building". The first implementation of it can be found in ${page.pcitCppVersion("v0.0.82.0")}. However, it was a bit flawed and ${page.pcitCppVersion("v0.0.83.1")} is a better representation of how it works (it is also the latest version at time of writing). It was significantly simpler to implement, which is I think is a good sign. The only road block was getting the circular dependency detection to never deadlock or race-condition. The solution I came up with for that is not one I'm super happy with (performance wise), but that's something I think I can come back to in the future. I hope these aren't famous last words.`);

			page.paragraph(`In ${page.pcitCppVersion("v0.0.83.1")}, variables, ${html.highlight("@import")} calls, ${terms.get("when conditionals")}, the beginnings of the standard library, and dynamic loading of files through ${html.highlight("@import")} all work. My plan is to implement all of the potential pain points of the new Symbol Process Building as fast as possible to make sure this new system works as well as it seems it does.`);
		}
	);
}

...
// TODO: move this to somewhere publicly accessible?
function pcit_cpp_version(version){
	return html.inline_code(html.link(version, "https://github.com/PCIT-Project/PCIT-CPP/blob/main/CHANGELOG.md#" + version));
}


page.paragraph(`In ${html.link("my first devlog", "/site/devlog/new_systems_requires_major_changes.html")}, I discussed how implementing Order Independent Declarations and the Panther build system required redoing a lot of work for the compiler as well as adding a new stage (Dependency Analysis). Update ${pcit_cpp_version("v0.0.79.0")} contained the first proof of concept of this new system working. The code required to get it working was complex and difficult to understand, but the system worked and it seemed robust. There was a looming sense of difficulty coming down the line (such as pointers to user-defined types - more on that later), but I plowed through as this was the solution I came up with and I was sticking to it. Through ${pcit_cpp_version("v0.0.80.0")} and ${pcit_cpp_version("v0.0.81.0")} it continued to work well, albeit with more and more complexity. I then got to implementing ${html.inline_code("@import")} and ran face-first into a wall made of pure abstract complexity.`);

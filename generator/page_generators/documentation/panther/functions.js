...
page.paragraph(`If a function explicit return parameters, a return statement in that function must be "${page.inline_code_block(Language.PANTHER, "return...;")}". Likewise, if a function has explicit error parameters, an error statement in that function must be "${page.inline_code_block(Language.PANTHER, "error...;")}". The "${page.inline_code_block(Language.PANTHER, "...")}" is a separate token which means there may be white-space in-between it and the respective keyword, although this goes against the ${terms.get("Panther style guide")}.`);

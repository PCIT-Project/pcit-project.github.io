...
exports.getPageGenerator = function(){
	return new (require("../../../PageGenerator.js").PageGenerator)(
		...
		(page) => {
			...
			page.paragraph(`Syntaxes 1, and 3 create a function that has a single return value. A return type of ${page.inlineCode("Void")} means that the function doesn't return anything. Syntaxes 2 and 4 have explicit return parameters, which allows returning of multiple values. Syntaxes 3 and 4 are for functions that may ${html.link("error", "#error")}.`);

			page.paragraph(`The list of parameters, return parameters, and error parameters are delimited by a comma (${html.highlight(",")}). A comma may also go at the end of a parameter even if there is no following parameter(s).`);
			...
			page.paragraph(`If a function explicit return parameters, a return statement in that function must be "${page.inlineCode("return...;")}". Likewise, if a function has explicit error parameters, an error statement in that function must be "${page.inlineCode("error...;")}". The "${page.inlineCode("...")}" is a separate token which means there may be white-space in-between it and the respective keyword, although this goes against the ${terms.get("Panther style guide")}.`);

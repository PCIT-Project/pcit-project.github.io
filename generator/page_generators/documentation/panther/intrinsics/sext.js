	...
	return new (require("../../../../PageGenerator.js").PageGenerator)(
		() => {
			return new Page.Page(__filename, {
				path                : "documentation/panther/intrinsics/sext.html",
				title               : "@sext",
				...
				on_site_description : `Sign extend any ${terms.get("integral")} type to any other larger ${terms.get("integral")} type`,
				symbol_kind         : Page.SymbolKind.INTRINSIC_FUNCTION,
			})
		},
		(page) => {
			page.addSymbolDecls(["func @sext = <{FROM: Type, TO: Type}> (from: FROM) -> TO;"]);

			page.addSymbolDescription(`Sign extend any ${terms.get("integral")} type to any other larger ${terms.get("integral")} type. Requires that ${page.inlineCode("@numBits<{FROM, false}>() < @numBits<{TO, false}>()")}.`);
			...
			page.addSymbolSeeAlso(["@zext", "@fext"]);
		}
	);
}

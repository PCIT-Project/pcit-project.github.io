	...
	return new (require("../../../../PageGenerator.js").PageGenerator)(
		() => {
			return new Page.Page(__filename, {
				path                : "documentation/panther/intrinsics/ftrunc.html",
				title               : "@ftrunc",
				...
				on_site_description : `Truncate any ${terms.get("floating-point")} type to any other smaller ${terms.get("floating-point")} type`,
				symbol_kind         : Page.SymbolKind.INTRINSIC_FUNCTION,
			})
		},
		(page) => {
			page.addSymbolDecls(["func @ftrunc = <{FROM: Type, TO: Type}> (from: FROM) #unsafe -> TO;"]);

			page.addSymbolDescription(`Truncate any ${terms.get("floating-point")} type to any other smaller ${terms.get("floating-point")} type. Requires that ${page.inlineCode("@numBits<{FROM, false}>() > @numBits<{TO, false}>()")}.`);

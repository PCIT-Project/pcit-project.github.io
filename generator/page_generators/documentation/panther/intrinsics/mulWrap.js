	...
	return new (require("../../../../PageGenerator.js").PageGenerator)(
		() => {
			return new Page.Page(__filename, {
				path                : "documentation/panther/intrinsics/mulWrap.html",
				title               : "@mulWrap",
				...
				on_site_description : "Integer multiplication that returns if operation wrapped",
				symbol_kind         : Page.SymbolKind.INTRINSIC_FUNCTION,
			})
		},
		(page) => {
			page.addSymbolDecls(["func @mulWrap = <{T: Type}> (lhs: T, rhs: T) -> (result: T, wrapped: Bool);"]);

			page.addSymbolDescription(`Integer multiplication that returns if operation wrapped.`);
			...
			page.addSymbolReturnParams([
				new Page.ReturnParam("result", "result of the multiplication"),
				new Page.ReturnParam("wrapped", `${page.inlineCode("true")} if the operation wrapped`),
			]);


			page.addSymbolExampleTodo();

			page.addSymbolSeeAlso(["@mul", "@mulSat", "@fmul"]);
		}
	);
}

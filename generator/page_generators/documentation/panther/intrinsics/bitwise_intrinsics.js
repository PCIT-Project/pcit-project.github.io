...
page.text(`Bitwise shift left. ${html.inline_code("T")} must be ${terms.get("integral")} or a ${terms.get("vector")} of ${terms.get("integral")}. ${html.inline_code("SHIFT_T")} must be ${terms.get("unsigned integral")} with a width of ${page.inline_code_block(Language.PANTHER, "ceil(log2(@numBits<{T, false}>()))")}`);
page.text(`If ${html.inline_code("T")} is ${terms.get("unsigned integral")} and ${html.inline_code("MAY_WRAP")} is ${page.inline_code_block(Language.PANTHER, "false")}, it is ${terms.get("undefined behavior")} if any ${html.inline_code("1")} bits are shifted out.`);
page.text(`If ${html.inline_code("T")} is ${terms.get("signed integral")} and ${html.inline_code("MAY_WRAP")} is ${page.inline_code_block(Language.PANTHER, "false")}, it is ${terms.get("undefined behavior")} if the result is a different sign than lhs.`);
...
page.text(`Bitwise saturating shift left. ${html.inline_code("T")} must be ${terms.get("integral")} or a ${terms.get("vector")} of ${terms.get("integral")}. ${html.inline_code("SHIFT_T")} must be ${terms.get("unsigned integral")} with a width of ${page.inline_code_block(Language.PANTHER, "ceil(log2(@numBits<{T, false}>()))")} (${html.italic("this restriction most likely will change in the future, but more consideration is required")}).`);
...
page.text(`Bitwise shift right. ${html.inline_code("T")} must be ${terms.get("integral")} or a ${terms.get("vector")} of ${terms.get("integral")}. ${html.inline_code("SHIFT_T")} must be ${terms.get("unsigned integral")} with a width of ${page.inline_code_block(Language.PANTHER, "ceil(log2(@numBits<{T, false}>()))")}.`);
...
page.text(page.inline_code_block(Language.PANTHER, "func @byteSwap = <{T: Type}> (arg: T) -> T;"));
